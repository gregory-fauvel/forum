...
if($resultatget["user_id"] != $_SESSION["id"]){
    $requestlike = "INSERT INTO interaction(user_id, message_id, aime, aimepas) VALUES ($iduser, $id, 1, 0)";
    $querylike = mysqli_query($connect,$requestlike);
    header("Location:../message.php?id=$idconv");
}
if($resultatget["user_id"] == $_SESSION["id"]){
    $requestlikeupdate = "UPDATE `interaction` SET `aime`= 1,`aimepas`= 0 WHERE user_id = $iduser";
